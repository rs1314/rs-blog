...
<head>
    ...
    <title>${groupTopic.title} - ${groupTopic.group.name} - ${SITE_NAME} - Powered By rs</title>
    ...
    <script src="${basePath}/res/front/js/rs.js"></script>
    <script>
        var base = "${basePath}";
                                    ...
                                    <ul class="dropdown-menu">
                                        ...
                                        <#if loginUser?? && loginUser.isAdmin &gt; 0>
                                            <#if groupTopic.isTop = 0>
                                                <li><a href="${groupPath}/topic/top/${groupTopic.id}?top=1" target="_rsLink" callback="reload">普通置顶</a></li>
                                                <li><a href="${groupPath}/topic/top/${groupTopic.id}?top=2" target="_rsLink" callback="reload">超级置顶</a></li>
                                            <#elseif groupTopic.isTop = 1>
                                                <li><a href="${groupPath}/topic/top/${groupTopic.id}?top=0" target="_rsLink" callback="reload">取消普通置顶</a></li>
                                                <li><a href="${groupPath}/topic/top/${groupTopic.id}?top=2" target="_rsLink" callback="reload">超级置顶</a></li>
                                            <#elseif groupTopic.isTop = 2>
                                                <li><a href="${groupPath}/topic/top/${groupTopic.id}?top=0" target="_rsLink" callback="reload">取消超级置顶</a></li>
                                                <li><a href="${groupPath}/topic/top/${groupTopic.id}?top=1" target="_rsLink" callback="reload">普通置顶</a></li>
                                            </#if>
                                            <#if groupTopic.isEssence = 0>
                                                <li><a href="${groupPath}/topic/essence/${groupTopic.id}?essence=1" target="_rsLink" callback="reload">精华</a></li>
                                            <#elseif groupTopic.isEssence = 1>
                                                <li><a href="${groupPath}/topic/essence/${groupTopic.id}?essence=0" target="_rsLink" callback="reload">取消精华</a></li>
                                            </#if>
                                        </#if>
                                        ...
                                        <li><a href="${groupPath}/delete/${groupTopic.id}" confirm="确定要删除帖子吗？" target="_rsLink" callback="deleteSuccess">删除</a></li>
                                    </ul>
                                </div>
                            </#if>
                            </dd>
                ...
                <div class="comments panel">
                    ...
                    <header>
                        <div class="reply-form">
                            <form class="form-horizontal rs_form"
                                ...
                                <div class="form-group comment-user">
                                    <input type="submit" value="评论" class="pull-right btn btn-primary mg-t-10 rs-submit">
                                </div>
                            </form>
                        </div>
                    </header>
                            ...
                            <span class="text-right">
                                <#if isfollow == true>
                                    <a title="取消关注" href="${groupPath}/nofollow/${groupTopic.group.id}"
                                       target="_rsLink" callback="reload"><i class="icon-minus"></i> 取消关注</a>
                                <#else>
                                    <a title="添加关注" href="${groupPath}/follow/${groupTopic.group.id}"
                                       target="_rsLink" callback="reload"><i
